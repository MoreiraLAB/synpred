...
def generate_features_file(input_cell_line, input_drug_A, input_drug_B):
    ...
    all_features = pd.concat([drug_features, final_genomic_features], axis = 1).drop(["index","cells"], axis = 1)
    return all_features

def generate_features_dataframe(input_table, cell_col = "Cell", Drug1_col = "Drug1", Drug2_col = "Drug2", verbose = True):
    ...
    unique_cell_list = list(input_table[cell_col].unique())
    cell_lines_list = standalone_variables.process_cells_file(CELL_LINES_FILE, \
                                    mode = "standalone",  \
                                    target_column = CELL_LINES_TYPE,  \
                                    input_cell_type = unique_cell_list)
    
    cell_features_5 = empty_zeros(subset_table_rows(cell_lines_list, CELL_LINES_5).add_prefix("CCLE_5_"), cell_col)
    cell_features_5.rename(columns={'CCLE_5_cells':'cells'}, inplace=True)
    cell_features_6 = empty_zeros(subset_table_rows(cell_lines_list, CELL_LINES_6).add_prefix("CCLE_6_"), cell_col)
    cell_features_6.rename(columns={'CCLE_6_cells':'cells'}, inplace=True)
    cell_features_7 = empty_zeros(subset_table_rows(cell_lines_list, CELL_LINES_7).add_prefix("CCLE_7_"), cell_col)
    cell_features_7.rename(columns={'CCLE_7_cells':'cells'}, inplace=True)
    cell_features_12 = empty_zeros(subset_table_rows(cell_lines_list, CELL_LINES_12).add_prefix("CCLE_12_"), cell_col)
    cell_features_12.rename(columns={'CCLE_12_cells':'cells'}, inplace=True)
    cell_features_14 = empty_zeros(subset_table_rows(cell_lines_list, CELL_LINES_14).add_prefix("CCLE_14_"), cell_col)
    cell_features_14.rename(columns={'CCLE_14_cells':'cells'}, inplace=True)
    cell_features_17 = empty_zeros(subset_table_rows(cell_lines_list, CELL_LINES_17).add_prefix("CCLE_17_"), cell_col)
    cell_features_17.rename(columns={'CCLE_17_cells':'cells'}, inplace=True)
    cell_features_22 = empty_zeros(subset_table_rows(cell_lines_list, CELL_LINES_22).add_prefix("CCLE_22_"), cell_col)
    cell_features_22.rename(columns={'CCLE_22_cells':'cells'}, inplace=True)
    common_cell_lines = locate_common([cell_features_5, cell_features_6, cell_features_7, \
                                        cell_features_12, cell_features_14, cell_features_17, \
                                        cell_features_22])
    ...
    unique_drugs = list(input_table[Drug1_col].unique()) + list(input_table[Drug2_col].unique())
    drug_features_dictionary = {}
    for index, current_drug in enumerate(unique_drugs):
        if verbose == True:
            print("Retrieving features for drug:", index + 1, "/", len(unique_drugs))
        drug_features_dictionary[current_drug] = pd.DataFrame(drug_features_extractor(current_drug)).transpose()
    
    cell_features_list, drug_A_features_list, drug_B_features_list = [], [], []
    cell_log_dictionary = {}
    missed_indexes = []
    for index, row in input_table.iterrows():
        
        cell_features_current_df = final_genomic_features.loc[final_genomic_features[CELL_LINES_COLUMN] == row[cell_col]].drop(["cells"], axis = 1)
        drug_1_current_df = drug_features_dictionary[row[Drug1_col]]
        drug_2_current_df = drug_features_dictionary[row[Drug2_col]]
        if (cell_features_current_df.empty == False) and (drug_1_current_df.empty == False) and (drug_2_current_df.empty == False):
            cell_features_list.append(list(cell_features_current_df.values[0]))
            drug_A_features_list.append(list(drug_1_current_df.values[0]))
            drug_B_features_list.append(list(drug_2_current_df.values[0]))
        else:
            cell_features_list.append([0]*175)
            drug_A_features_list.append([0]*586)
            drug_B_features_list.append([0]*586)
            missed_indexes.append(index)

        if verbose == True:
            print("Processing row:", index + 1, "/", input_table.shape[0])

    drug_A_features = pd.DataFrame(drug_A_features_list)
    drug_B_features = pd.DataFrame(drug_B_features_list)
    cell_features = pd.DataFrame(cell_features_list)
    return pd.concat([drug_A_features,drug_B_features,cell_features], axis = 1), missed_indexes