...
def make_ds(file='', method='ZIP', c_type='all', sample_fraction = 1.0):
    ...
    #droper = ['cell', 'drug1', 'drug2', 'ZIP', 'Bliss', 'HSA', 'Loewe','Full-agreement']
...
def model_training(ds, method, input_model, c_type='all', outp=None, save_model = True):

    names, features, target = make_ds(file=ds, method=method, c_type=c_type, sample_fraction = 0.1)
    history = input_model.model.fit(x = features, y = target, epochs = 150, validation_split=0.20)
    if save_model == True:
...
method = "ZIP"
train = "./datasets/train_autoencoder_custom_drugs.csv"
test = "./datasets/test_autoencoder_custom_drugs.csv"
outp = "custom_drug_test"
input_architecture = [100,100,100]
#input_features = 4229 # autoencoder
#input_features = 4446 # PCA 
#input_features = 4621 # PCA custom
input_features = 7503

raw_model =  neural_network_class(input_architecture, input_features, dropout_rate = 0)
optimizer = tf.keras.optimizers.Adam(0.0001)

raw_model.model.compile(loss='binary_crossentropy', optimizer = optimizer, metrics=['accuracy'])
classification_model, train_class, train_predictions = model_training(train, method, raw_model, c_type=c_type, outp=outp, save_model = False)

names, test, target = make_ds(file = test, method=method, c_type=c_type)
