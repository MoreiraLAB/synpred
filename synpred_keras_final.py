...
from synpred_variables import INTERMEDIATE_SEP, DL_SAVED_MODELS, \
                                SYSTEM_SEP, RANDOM_STATE, CELL_TYPES, \
                                DROPPABLE_COLUMNS
...
def model_training(dataset, method, input_model, c_type='all', \
                    outp = None, save_model = True):

    """
    Train the dataset with Keras using the full training set
    """
    names, features, target = prepare_dataset(file = dataset, method = method, c_type = c_type)
...
classification_model, train_class, train_predictions = model_training(train, \
                                                    method, raw_model, \
                                                    c_type = c_type, outp = outp, save_model = True)
