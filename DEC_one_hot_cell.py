...
from DEC_variables import SYSTEM_SEP, CSV_SEP, PARAGRAPH_SEP, \
							...
							TRAIN_DATASET_ONE_HOT_CELL, TEST_DATASET_ONE_HOT_CELL, \
							SCALER_MORDRED_FILE_ONE_HOT, TRAIN_DATASET_NO_DRUG, \
							TEST_DATASET_NO_DRUG
...
def edit_file(input_file, output_name, class_table, mordred_dataset_file = MORDRED_RAW_FILE, \
	...
	scaler_name = REDEPLOYMENT_FOLDER + SYSTEM_SEP + SCALER_MORDRED_FILE_ONE_HOT
	if mode == "train":
		...
		#output_dataframe = pd.concat([final_ids, cells_dataframe, final_features], axis = 1)
...
edit_file(TRAIN_DATASET, TRAIN_DATASET_NO_DRUG, CLASS_TRAIN_DATASET, mode = "train")
edit_file(TEST_DATASET, TEST_DATASET_NO_DRUG, CLASS_TEST_DATASET, mode = "other")