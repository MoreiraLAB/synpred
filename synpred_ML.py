...
from synpred_variables import SYSTEM_SEP, PARAGRAPH_SEP, CSV_SEP, \
                            ...
                            POSSIBLE_TARGETS, DATASETS_DICTIONARY_DRUGCOMB, POSSIBLE_TARGETS_DRUGCOMB, ML_DRUGCOMB_GRIDSEARCH_RESULTS_PROCESSED
...
def deploy_ML_pipeline(preprocessing, input_data_dictionary, input_parameters_dictionary, \
                        ...
                        verbose = True, save_model = True, other_info = ""):
    ...
    for current_target in POSSIBLE_TARGETS_DRUGCOMB:
        ...
        for current_method in ML_dictionary.keys():
            ...
            model_name = REDEPLOYMENT_FOLDER + SYSTEM_SEP + output_name + other_info + ".pkl"
            if verbose == True:
            ...
            model_evaluation(current_data_dictionary["train_class"], predicted_train, \
                                verbose = True, write_mode = True, subset_type = output_name  + other_info + INTERMEDIATE_SEP + "train" + INTERMEDIATE_SEP + current_target_name, \
                                task_type = problem_type)

            model_evaluation(current_data_dictionary["test_class"], predicted_test, \
                                verbose = True, write_mode = True, subset_type = output_name + other_info + INTERMEDIATE_SEP + "test" + INTERMEDIATE_SEP + current_target_name, \
                                task_type = problem_type)
...
ML_gridsearch_results = open_gridsearch_results(input_file = ML_DRUGCOMB_GRIDSEARCH_RESULTS_PROCESSED)
...
deploy_ML_pipeline(preprocessing_mode, DATASETS_DICTIONARY_DRUGCOMB, \
                    ML_gridsearch_results, ML_dictionary_classification, \
                    ML_dictionary_regression, save_model = True, other_info = "drugcomb_")