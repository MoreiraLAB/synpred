...
from DEC_variables import SYSTEM_SEP, CSV_SEP, PARAGRAPH_SEP, \
							...
							REDEPLOYMENT_FOLDER, SCALER_CUSTOM_FILE, \
							CLASS_TRAIN_DATASET, CLASS_TEST_DATASET, \
							TRAIN_DATASET_ONE_HOT_CELL, TEST_DATASET_ONE_HOT_CELL, TRAIN_DATASET_NO_DRUG, \
							TEST_DATASET_NO_DRUG, CUSTOM_MOLECULES_FEATURES, TRAIN_DATASET_CUSTOM_DRUG, \
                            TEST_DATASET_CUSTOM_DRUG

from sklearn.preprocessing import StandardScaler
import pickle

def edit_file(input_file, output_name, class_table, custom_dataset_file = CUSTOM_MOLECULES_FEATURES, \
	...
	"""
	opened_file = pd.read_csv(input_file, header = 0).drop(["Unnamed: 0"], axis = 1)
	...
	opened_custom_file = pd.read_csv(custom_dataset_file, header = 0)
	drug1_dataframe = opened_custom_file.add_prefix("drug1_")
	drug1_dataframe["drug1"] = drug1_dataframe["drug1_drug"]

	drug1_dataframe = drug1_dataframe.drop(["drug1_drug"], axis = 1)
	joint_dataframe = only_ids_and_class.merge(drug1_dataframe, on = "drug1", how = "left")

	drug2_dataframe = opened_custom_file.add_prefix("drug2_")
	drug2_dataframe["drug2"] = drug2_dataframe["drug2_drug"]
	drug2_dataframe = drug2_dataframe.drop(["drug2_drug"], axis = 1)
	...
	#scaler_name = REDEPLOYMENT_FOLDER + SYSTEM_SEP + SCALER_CUSTOM_FILE
	if mode == "train":
		#scaler = StandardScaler()
		#scaler.fit(only_features)
		#final_features = pd.DataFrame(scaler.transform(only_features))
		final_features = only_features
		final_features.columns = features_name
		cells_dataframe = pd.get_dummies(final_ids, prefix = ["cell"], columns = ["cell"], drop_first = False)
		output_dataframe = pd.concat([final_ids["cell"], cells_dataframe, final_features], axis = 1).fillna(0)
		#output_dataframe = pd.concat([final_ids, cells_dataframe], axis = 1)
...
edit_file(TRAIN_DATASET, TRAIN_DATASET_CUSTOM_DRUG, CLASS_TRAIN_DATASET, mode = "train")
edit_file(TEST_DATASET, TEST_DATASET_CUSTOM_DRUG, CLASS_TEST_DATASET, mode = "other")